...
# ----------------------------
# Stable device paths (edit to match your wiring)
# ----------------------------
...
MAIN_FPS = 2.0                 # main cameras
CONTROL_SAVE_EVERY_S = 0.5     # control cam recording (while session active)
IMU_HZ = 100                    # IMU logging rate (start with 50)

# QR debounce
QR_SCAN_FPS = 6
QR_CONSEC_REQUIRED = 5
QR_COOLDOWN_S = 8.0
...
class OLEDUI:
    ...
    def render(self, snap: Status):
        line1 = f"{snap.state[:10]:<10} QR:{snap.last_qr or '-'}"
        cam_tot = len(snap.cams)
        if snap.state != "RECORDING":
            # In IDLE/ERROR, do not show stale "OK" from the last recording cycle
            cam_ok = 0
        else:
            cam_ok = sum(1 for ds in snap.cams.values() if ds.ok)

        line2 = f"CAM: {cam_ok}/{cam_tot}  CTRL:{'OK' if snap.control_cam.ok else 'NO'}"
        line3 = f"IMU:{'OK' if snap.imu.ok else 'NO'}"
        line4 = " " if not snap.error_msg else f"ERR:{snap.error_msg[:16]}"
        ...
        self.draw.text((0, 10), line2, font=self.font, fill=255)
        self.draw.text((0, 20), line3, font=self.font, fill=255)
        # 4th line doesn't fit cleanly on 32px w/ default font; keep it short or omit
        # self.draw.text((0, 30), line4, font=self.font, fill=255)
...
def init_ism330(bus) -> bool:
    ...
    # Commonly 0x6B for ISM330DHCX (you already verified)
        ...
        while not self.stop_all.is_set():
            with status_lock:
                snap = Status(
                    ...
                    control_cam=status.control_cam,
                    imu=status.imu,
                    cams=dict(status.cams),
